...
// ref. https://github.com/osmdroid/osmdroid/blob/master/osmdroid-android/src/main/java/org/metalev/multitouch/controller/MultiTouchController.java
...
//    private static final float MIN_MULTITOUCH_SEPARATION = 30.0f;
    ...
    public static final float SETTLE_THRESHOLD = 5.0f;
    ...
    public PointInfo getCurrentPoint() {return mCurrPt;}

    /** The previous touch point */
    private PointInfo mPrevPt;

    private PointInfo mInitPt;

    private PointInfo mLastUpPt;
    ...
    private T prevSelectedObject = null;
    ...
    private double startAngle, startRadius;
    ...
    public boolean dragOccurred() {return mDragOccurred;}
    ...
    public MultiTouchController(MultiTouchObjectCanvas<T> objectCanvas, boolean handleSingleTouchEvents) {
        ...
        this.mInitPt = new PointInfo();
        this.mLastUpPt = new PointInfo();
...
//            return true;
    ...
    private void anchorAtThisPositionAndScale() {
        ...
        startAngle = mCurrPtAng + Math.atan2(startPosY, -startPosX);
        startRadius = Math.sqrt(startPosX*startPosX + startPosY+startPosY);
...
//        float newPosY = mCurrPtY - startPosY * currScale;
        ...
        // custom - rotation in center
        double diffAngle = startAngle - mCurrPtAng;
        float newPosX = (float)(mCurrPtX + startRadius * currScale * Math.cos(diffAngle));
        float newPosY = (float)(mCurrPtY - startRadius * currScale * Math.sin(diffAngle));
        ...
        switch (mMode) {
            ...
            case MODE_DRAG:
            {
                if (!mCurrPt.isDown()) {
                    ...
                    boolean performDoubleTab = false;
                    if (prevSelectedObject!=null && prevSelectedObject==selectedObject) {
                        if (mCurrPt.getEventTime() - mLastUpPt.getEventTime() < AppConst.Config.DOUBLE_TAP_TIMEOUT) {
                            Vec2 p1 = objectCanvas.toWorldPoint(new Vec2(mCurrPt.getPoint()));
                            Vec2 p2 = objectCanvas.toWorldPoint(new Vec2(mLastUpPt.getPoint()));
...
//                        mLastUpPt.set(mCurrPt);
                    ...
                    if (mCurrPt.getEventTime() < mSettleEndTime) {
                        ...
                        if ((float)Math.sqrt(deltaX*deltaX + deltaY+deltaY) > SETTLE_THRESHOLD) {
                            performDragOrPinch();
...
//    }
    ...
    public static class PointInfo {
        ...
        public final Vec2 getPoint() {return new Vec2(getX(), getY());}
        ...
        public float getDistance(PointInfo other) {
    ...
    public static interface MultiTouchObjectCanvas<T> {
        ...
        public void doubleClickObject(T obj, PointInfo touchPoint);

        public void touchModeChanged(final int touchMode, PointInfo touchPoint);
